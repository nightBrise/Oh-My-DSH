...
| **Auto-open right pane after dispatch** | Subscribes to `sessions.list` and detects a new child (`parentId` = current session) → `refreshSubagents` + `openDetails()` (idempotent, no open/close bookkeeping); **`seatReady` gate**: no pop-up until the webui-enhance sub-seat has been injected |
