...
const DEFAULT_TYPES = {
  ...
  review: {
    tier: 'pro',
    tools: { allow: ['read', 'glob', 'grep', 'web_search', 'skill', 'list_agents', 'job_list', 'job_output', 'get_goal', 'bash'] },
    role: 'independent review of the latest changes: FIRST run `git status` and `git diff HEAD` (and `git diff --stat HEAD`) to see exactly what changed, then review those diffs for quality/security/performance/edge cases; report prioritized issues with concrete fixes and file:line references. Use bash ONLY for read-only git inspection — never modify files or run mutating commands.',
  },
}
const DEFAULT_PERSONAS = {
  ...
  planner:    { text: 'For this task you act as a task planner: break a complex requirement into 2-5 concrete subtasks, each with EARS-style acceptance criteria (WHEN/WHILE/WHERE/IF ... THE SYSTEM SHALL ...), explicit file paths, dependencies, boundaries, and non-goals. Plan only — never modify any file.' },
  consultant: { text: 'For this task you act as a senior consultant: analyze the most important architectural or technical decisions with structured reasoning, compare candidate options with risks, and give a clear recommendation with reasons. Read-only advisory — never implement.' },
}
...
export function apply(ctx) {
  ...
  function protocolText() {
    ...
    return [
      ...
      '定位，绝不新开子代理去"找回"旧任务。',
      '',
      '### 委托决策（Delegation decision）',
      '何时委派：满足任一条件才派子代理——',
      '  ① 工具边界：任务需要独立工具边界（只读调研、文件写作、独立审查、代码实现），',
      '     让子代理带着自己的边界干活，不污染主对话工具流；',
      '  ② 并行：多个相互独立的子任务需要同轮并行展开（见 fan-out）；',
      '  ③ 上下文隔离：任务细节多、过程长，会挤占主对话，值得隔离消化；',
      '  ④ 专业人设：任务匹配 persona 库中的专业身份（key 引用）。',
      '  不委派：简单查询、可内联完成、几句话能说清、或答案已在主对话——自己做。',
      '  判据速问：这个任务会不会显著占用我的上下文或工具流？不会 → 不委派。',
      '',
      '长驻 vs 一次性（run_in_background）：',
      '  常驻 true：需要多轮交互才能收敛、结果要跨轮复用、或属持续协作（如"持续调研 xxx"）',
      '    ——返回 subagentId，之后用 send_message 续接；',
      '  一次性 false：任务边界明确、一次产出即结束（一次研究/实现/审查）——默认选项；',
      '  结构化返回：需要可机读结果（合并/计算/入库）→ 只能前台（output_schema）。',
      '  判据速问：我之后还会需要再找它吗？会 → 常驻；不会 → 一次性。',
      '',
      '角色与档位：type 按任务性质选（工具边界+输出约定+默认档位），不按偏好；',
      '  tier 仅在不满足于 type 默认档时显式覆盖（更贵=更强判断，更便宜=省成本）；',
      '  persona 仅当任务需要该专业身份时引用，非必要不用。',
      '',
      '成员复用（Team lifecycle）：同类任务优先续接现有常驻成员（label/id 定位 + send_message），',
      '  绝不新开子代理去"找回"旧任务；任务性质改变、或旧成员已退役，才新建成员；',
      '  续接前核对 list_agents：休息成员直接续接，工作中成员等其空闲或说明取舍。',
      '',
      ...
      '',
      '### 档位决策规则（成本优先，L）',
      '档位是成本轴——默认用便宜的，升档必须给理由：',
      '- lite（默认）：边界清晰、验收明确的常规任务——调研/检索/明确实现/整理/摘要；',
      '  凡能用 lite 完成的一律 lite，禁止无理由升档；',
      '- standard：需要强推理、高风险、核心路径、多文件联动的任务（复杂实现/拆解规划）；',
      '  规划拆解默认 standard；',
      '- pro：对抗验收（review 类型默认）、较难审查；',
      '- ultra：最重要决策、架构仲裁、终局审查——只在刀刃上，日常不用；',
      '- 不确定档位时：先 lite 试跑，结果不足再升档（升档比降档安全）；',
      '- 一次 fan-out 内优先保证 lite 任务占多数，控制总预算。',
      '',
      '### 并行与复用纪律（效率，E）',
      '- 独立子任务必须并行：同一轮 fan-out 全部发出，禁止串行硬做；只有数据依赖才分批；',
      '- 同类任务复用现有常驻成员：label/id 定位 + send_message 续接，禁止冷启动"找回"；',
      '- 返工/修订/追问一律 send 复用原会话（保留上下文），绝不重新 dispatch 冷启动；',
      '- 前台一次性失败后如需修订，改用 run_in_background=true 创建常驻成员承接后续。',
      '',
      '### 验收闭环（maker/checker 分离，软约束）',
      '- code 类型任务完成后，必须派 review 类型验收（独立子代理、pro 档、只读 + git 检视）；',
      '- reviewer 以 `git diff HEAD`（含 git status / git diff --stat）为审查对象——先看清实际改动再审查，',
      '  不要凭空审查；派 review 时 prompt 应说明"审查最近的更改（git diff HEAD）"；',
      '- reviewer 判 FAIL → 修订最多 2 轮（send 原实现者）→ 仍 FAIL 则显式上报用户或派 consultant 仲裁；',
      '- 验收标准在实现中不得修改（契约冻结）；reviewer 的 uncertain 项必须处理，不得静默放行。',
      '',
      '### EARS 验收标准规范（Spec 是仲裁者）',
      '- 复杂任务的验收标准用 EARS 语法：WHEN/WHILE/WHERE/IF <条件> THE SYSTEM SHALL <行为>；',
      '  如"WHEN 收到 GET /api/me 且携带有效 token，THE SYSTEM SHALL 返回 200 与用户信息 JSON"；',
      '- 逐条编号、可执行验证（命令/测试/肉眼检查），禁止"代码质量好""实现正确"这类主观表述；',
      '- 明确不做什么（防 scope creep）；标准一旦冻结，实现与验收都以它为准。',
      '',
      '### 团队协议骨架（D28 星型团队）',
  ...
  ctx.tools.register(defineTool({
    name: 'dispatch',
    description: 'Delegate a task to a subagent as the sole delegation entry. type is a closed whitelist (explore/code/write/review) deciding tool boundary, role, and default tier; tier overrides the cost tier (lite=MiMoV2.5Pro/standard=DeepSeekV4Flash/pro=DeepSeekV4Pro/ultra=Qwen3.8Max); persona is a persona-library key (physics/ml/data/research/docs/backend/reviewer/statistician/planner/consultant) or free text; run_in_background true creates a persistent member returning subagentId; output_schema (foreground only) returns validated JSON or null; timeout in seconds (foreground default none / max 3600, background default 900). Delegation is governed by this tool; native subagent/workflow tools are restricted for children (D28 star team).',
    parameters: {
